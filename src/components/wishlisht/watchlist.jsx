...
const WatchlistPage = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      const q = query(collection(db, "watchlist"), where("category", "==", category));  // Create a query for Firestore
  ...
  const changeCategory = async (movieId, newCategory) => {
    try {
      ...
      setMovies((prevMovies) => prevMovies.filter(movie => movie.id !== movieId));  // Remove the movie from the current list
  ...
  return (
    <div>
      ...
      <div>
        <button onClick={() => setCategory("Currently Watching")}>Currently Watching</button>
        <button onClick={() => setCategory("Watched")}>Watched</button>
        <button onClick={() => setCategory("Already Watched")}>Already Watched</button>
      </div>
      <div>
        {movies.map((movie) => (
          <div key={movie.id}>
            <h3>{movie.title}</h3>
            <button onClick={() => changeCategory(movie.id, "Watched")}>Change to Watched</button>
            <button onClick={() => changeCategory(movie.id, "Already Watched")}>Change to Already Watched</button>
          </div>
        ))}
      </div>
    </div>
